...
#   - Agent (native): .agent/skills/ symlink + AGENTS.md (native)
...
show_help() {
    ...
    echo "  --agent     Configure Agent (native)"
...
show_menu() {
    ...
    local options=("Agent (native)" "Claude Code" "Gemini CLI" "Codex (OpenAI)" "GitHub Copilot")
    local selected=(true false false false false)  # Agent selected by default
...
setup_agent() {
    ...
    echo -e "${GREEN}  ✓ Agent uses AGENTS.md natively from root${NC}"
}
...
if [ "$SETUP_AGENT" = false ] && [ "$SETUP_CLAUDE" = false ] && [ "$SETUP_GEMINI" = false ] && [ "$SETUP_CODEX" = false ] && [ "$SETUP_COPILOT" = false ]; then
    show_menu
    echo ""
fi
...
STEP=1
TOTAL=0
[ "$SETUP_AGENT" = true ] && TOTAL=$((TOTAL + 1))
...
if [ "$SETUP_AGENT" = true ]; then
    echo -e "${YELLOW}[$STEP/$TOTAL] Setting up Agent...${NC}"
    setup_agent
    STEP=$((STEP + 1))
fi
...
echo ""
echo "Configured:"
[ "$SETUP_AGENT" = true ]   && echo "  • Agent (native): AGENTS.md"
